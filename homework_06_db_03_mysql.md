...
Без -d, чтобы видеть консольный лог работы сервера, по мере того 
...
при упоре в IO. Должны быть следующие параметры, считаю:
  ...
  #(примерно треть от 1Gb у моей ВМ, но в качестве 
  #рекомендуемого значения указано 50-75%, в статье, кстати)  
